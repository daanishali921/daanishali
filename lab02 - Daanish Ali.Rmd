...
```{r}
apply(X, 1, na.rm = TRUE, FUN = sd)
apply(X, 2, na.rm = TRUE, FUN = sd)
```
...
flip_matrix <- function(x){
  if (dim(x)[1] > dim(x)[2]){
    a = matrix(my_reverse(x[length(x[ ,1]):1,]), nrow = dim(x)[1], ncol = dim(x)[2])
    a
  } else if (dim(x)[1] < dim(x)[2]) {
    b = matrix(my_reverse(x[ ,length(x[1, ]):1]), nrow = dim(x)[1], ncol = dim(x)[2])
    b
  } else {
    print("break")
  }
...
* Find the 5%ile of `v` and use the `qnorm` function to compute what it theoretically should be. Is the estimate about what is expected by theory? YES

```{r}
quantile(v, .05)
qnorm(0.05, avg_v, sd_v, log.p = FALSE)
##the estimates are very close together 
```
...
k = unlist(strsplit(lorem, ".", fixed = TRUE))
sample(k)
```
