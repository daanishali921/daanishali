...
```{r}
boop = list("guitar" = c(1,2,3,4), "band" = c('LZ','MZ'))
boop$guitar[4]
boop[1]
...
Imagine you are running an experiment with many manipulations. You have 14 
levels in the variable "treatment" with levels a, b, c, etc. For each of those 
manipulations you have 3 submanipulations in a variable named "variation" with 
levels A, B, C. Then you have "gender" with levels M / F. Then you have "generation" 
with levels Boomer, GenX, Millenial. Then you will have 6 runs per each of these 
groups. In each set of 6 you will need to select a name without duplication from the 
appropriate set of names (from the last question). Create a data frame with columns 
treatment, variation, gender, generation, name and y that will store all the unique 
unit information in this experiment. Leave y empty because it will be measured as 
...
X$generation = rep(rep(c("Boomer", "GenX", "Millenial"), each = n / 14/ 3 /2 / 3), 
               times = 14 * 3 * 2)
...
g = c(2,2,2,2,22,2,2,2,2,2,2,22,2,2,2,3,1,1,2,5,6,7,8,9,5,7,8,5,3,2,3,4,5,67,8,8,6,4,3,5,7,9,9,4,3)
getmode(g)
```
...
Create the function `g_1` explicitly that can predict `y` from `x` being a new `Sepal.Length`.
...
#'
#' @param Xinput      Xinput are my input values/features -- data points
#' @param y_binary    This is the binary classification of our outputs
...
Let's first plot $y$ by the two features so the coordinate plane will be the two 
features and we use different colors to represent the third dimension, $y$.
...
This graph is linearly seperable. ggplot plots the data frame Xy_simple seperating the responses by color. Then, the x axis has first first feature while the y axis has the second feature.
...
w_vec_simple_per
#cbind(cbind(Xy_simple$first_feature, Xy_simple$second_feature))
```
...
Is this SVM line a better fit than the perceptron? No, they are the same
