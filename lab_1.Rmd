...
Yes it exactly follows the values of variable x 
...
```{r}
mean(sample(c(0,1), size = 500, replace = TRUE, prob = c(.01, .9)))
```
...
```{r}
x_3 <- c("none", "infraction", "misdimeanor", "felony")
x_3_char <- factor(sample(x_3, 100, replace = TRUE), ordered = FALSE, levels = x_3)
x_3_char
```
...
```{r}
x_3bin <- as.numeric(x_3_char != "none")
x_3bin
```
...
```{r}
x_3_ord = factor(sample(x_3, 100, replace = TRUE), levels = x_3, ordered = TRUE)
x_3_ord
```
